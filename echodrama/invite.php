...
$threadnum = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('mtag_thread')." WHERE uid='$invite[uid]'"), 0);
$mtagmembernum = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('mtagmember')." WHERE uid='$invite[uid]'"), 0);
