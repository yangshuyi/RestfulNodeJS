...
if(checkperm('managethread')) {
	...
	//群主
	if($tagid) {
		$grade = getcount('mtagmember', array('tagid'=>$tagid, 'uid'=>$_SGLOBAL['supe_uid']), 'grade');
		if($grade >= 8) {
			//是否成员
			$allowmanage = 1;
			$managebatch = 1;
...
if($perpage > 100) {
	...
	$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('mtag_thread')." WHERE $wheresql"), 0);
	$selectsql = '*';
}
...
$tags = $tagids = array();
if($count) {
	$query = $_SGLOBAL['db']->query("SELECT $selectsql FROM ".tname('mtag_thread')." WHERE $wheresql $ordersql LIMIT $start,$perpage");
