...
if(submitcheck('opsubmit')) {
	...
	if($_POST['optype'] == 'delete') {
		...
		if(!empty($_POST['ids']) && deleteevents($_POST['ids'])) {
			...
			cpmessage('choose_to_delete_the_columns_event'); // 请选择要删除的招募活动
		}
		...
		if(!empty($_POST['ids']) && verifyevents($_POST['ids'], $grade)) {
			...
			cpmessage('choose_to_grade_the_columns_event'); // 请选择要设置的招募活动状态
		}
	} else {
...
if(empty($_GET['op'])) {
	...
	//处理搜索
	$intkeys = array('eventid', 'uid', 'public', 'grade');
	$strkeys = array('province', 'city');
	$randkeys = array(array('intval','hot'));
	$likekeys = array('title', "classid");
...
function verifyevents($eventids, $grade){
	...
	if(!in_array($grade, array(-2,-1,1,2))){
		cpmessage('bad_event_grade');// 错误的招募活动状态
	}
	...
	foreach($newids as $id){
		$event = $events[$id];
		if($grade >= 1 && $events[$id]['grade'] < 1 && $events[$id]['grade'] >= -1 ){// feed:发布招募活动
			$poster = "";
			if(empty($event['poster'])){
				$poster = 'image/event/default.jpg';						
			} else {
