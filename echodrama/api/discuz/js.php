	...
	if(($_GET['pagetype'] == 'profile' && $viewpro) || ($_GET['pagetype'] == 'viewthread')) {
		...
		if($updateuid) {
			...
			if($_GET['pagetype'] == 'profile' && $viewpro) {
				...
				$threadnum = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('mtag_thread')." WHERE uid='$updateuid'"), 0);
				$mtagmembernum = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('mtagmember')." WHERE uid='$updateuid'"), 0);
				$contstr = '';
				...
				if($blognum)	$contstr .= '<li><img src="'.$siteurl.'image/icon/blog.gif"> <a href="'.$siteurl.'space.php?uid='.$updateuid.'&do=blog&view=me" target="_blank">'.$blognum.'篇日记</a></li>';
				...
				if($mtagmembernum)	$contstr .= '<li><img src="'.$siteurl.'image/icon/mtag.gif"> <a href="'.$siteurl.'space.php?do=mtag&uid='.$updateuid.'" target="_blank">'.$tagspacenum.'个群组</a></li>';
