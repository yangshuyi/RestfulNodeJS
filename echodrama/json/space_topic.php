...
if(empty($topicid) && empty($topicforecastid)) {
	...
	if($count>0) {
		...
		//显示分页
		$multi = multi_JSON($count, $perpage, $page, $mpurl);
	}
	...
	$_TPL['css'] = 'topic';
	$searchengine_description=$topic['typename'].":".$topic['subject'].",".$topic['productclassname'].",由".$topic['club']."发布,CAST:".$topic['cast'].",STAFF:".$topic['group'];
