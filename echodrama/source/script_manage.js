...
function post_add(pid, result) {
	if(result) {
		...
		var newli = document.createElement("div");
		var x = new Ajax();
		x.get('cp.php?ac=thread&op=loadpost&postid='+ pid, function(s){
			newli.innerHTML = s;
		});
		obj.appendChild(newli);
		//append or insert flag
		if(false){
			obj = obj.firstChild;
			while (obj && obj.nodeType != 1){
				obj = obj.nextSibling;
...
function post_edit(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('cp.php?ac=thread&op=loadpost&pid='+ pid, function(s){
			obj.innerHTML = s;
		});
	}
}
...
//资金注入
function injectcredit(id, result) {
	if(result) {
		hideMenu();
	}
}
...
function validate(){
	if(checkNumberError($("usercredit").value)){
		alert("用户的积分必须为数字！");
		$("credit").focus();
		return false;
	}

	if(checkNumberError($("credit").value)){
		alert("捐助资金必须为数字！");
		$("credit").focus();
		return false;
	}
	
	if( ( parseInt($('usercredit').value) ) < ( parseInt($('credit').value) ) ){
		alert("你的积分不够！");
		$("credit").focus();
		return false;
	}
...
function postinjectcreditwithajax(){
	if(validate()){
		ajaxpost('injectcreditform', 'injectcredit', 2000);
	}
}

function postinjectcreditwithnormal(){
	if(validate()){
		$('injectcreditform').submit();
	}
}
