...
if(submitcheck('addsubmit')) {
	...
	// 回调函数   
    include_once(S_ROOT.'./data/data_imageface.php');
       
    function convertsmile($matches) {   
    	global $_SGLOBAL;
    	return '('.$_SGLOBAL['imageface'][$matches[1]]['name'].')';   
    }   
	$outputmessage = preg_replace_callback("/\[em:(\d+):]/is", 'convertsmile', $message);
	$outputmessage = preg_replace("/\<br.*?\>/is", ' ', $outputmessage);
	...
	sendmessagetosina($_SGLOBAL['supe_uid'], $outputmessage."(应声虫)");
	
	include_once(S_ROOT.'./source/function_t_kaixin001.php');
	sendmessagetokaixin001($_SGLOBAL['supe_uid'], "应声虫-".$outputmessage);
