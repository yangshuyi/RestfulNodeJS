...
if(submitcheck('pollsubmit')) {
	...
	foreach($_POST['option'] as $key => $val) {
		$option = getstr(trim($val), 200, 1, 1, 1);
...
if($op == 'addopt') {
	...
	if(submitcheck('addopt')) {
		$newoption = getstr(trim($_POST['newoption']), 200, 1, 1, 1);
	...
	if(submitcheck('invitesubmittype')) {
		...
		elseif($_POST['invitesubmittype']==2){ //invite all friends
			...
			//
			getreward("poll_invite_friend", 1, $_SGLOBAL[supe_uid]);
			
		}elseif($_POST['invitesubmittype']==3){ //invite all members
			$friendsql="SELECT * FROM ".tname('member')." WHERE 1=1";
			$query = $_SGLOBAL['db']->query($friendsql);
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$ids[] = $value['uid'];
			}
			...
			//
			getreward("poll_invite_all", 1, $_SGLOBAL[supe_uid]);
		}
		if($ids) {
			//过滤已邀请的用户
		...
		showmessage('do_success', '/cp.php?ac=poll&op=invite&uid='.$poll['uid'].'&pid='.$pid);
	}
