...
// 更新活动分类
function jobclass_cache(){
    global $_SGLOBAL;

	$_SGLOBAL['jobclass'] = array();
	// 从数据库获取
	$query = $_SGLOBAL['db']->query("SELECT * FROM " . tname("jobclass") . " ORDER BY displayorder");
	while($value = $_SGLOBAL['db']->fetch_array($query)){
		$_SGLOBAL['jobclass'][$value['classid']] = $value;
	}
	cache_write('jobclass', "_SGLOBAL['jobclass']", $_SGLOBAL['jobclass']);
}
