...
function addEventWaterMark($srcfile){
	if(!file_exists($srcfile) || !$src_info = getimagesize($srcfile)) {
		return '';
	}
	$src_w = $src_info[0];
	...
	$watermarkfile = S_ROOT.'./image/event_water_mark.gif';
	$tempfile = $srcfile.'.temp.gif';
	...
	$widthrate=$src_w/$water_w;
	$heightrate=$src_h/$water_h;
	//压缩图片以适合水印分辨率
	$thumbsrcfile=makeeventthumb($srcfile,$water_w*2,$water_h*2, $tempfile);
	//添加水印
	makeeventwatermark($thumbsrcfile, $watermarkfile);
	//还原图片
	makeeventthumb($thumbsrcfile, $src_w, $src_h, $srcfile);
}
...
function maketopiccanvasthumb($type, $srcfile, $percent, $src_x, $src_y, $destW, $destH) {
	global $_SGLOBAL;
	$dstfile = $srcfile.'.'.$type.'.jpg';
	...
	if(function_exists("imagecreatetruecolor") && function_exists("imagecopyresampled") && @$ni = imagecreatetruecolor($destW, $destH)) {
		imagecopyresampled($ni, $im, 0, 0, $src_x, $src_y, $tow, $toh, $srcw, $srch);
	}
...
///人员招募完毕缩略图
function makeeventthumb($srcfile, $width, $height, $destFile) {
	global $_SGLOBAL;
	...
	//   showmessage($srcw."-".$srch."-".$width."-".$height."-".$tow."-".$toh);
	if(function_exists("imagecreatetruecolor") && function_exists("imagecopyresampled") && @$ni = imagecreatetruecolor($ftow, $ftoh)) {
		imagecopyresampled($ni, $im, 0, 0, 0, 0, $ftow, $ftoh, $srcw, $srch);
		//大图片
		if($make_max && @$maxni = imagecreatetruecolor($fmaxtow, $fmaxtoh)) {
...
//人员招募完毕水印
function makeeventwatermark($srcfile, $watermarkfile) {
	global $_SGLOBAL;
	...
	//    showmessage($src_w."-".$src_h."-".$water_w."-".$water_h);
	switch($src_info[2]) {
		case 1:
			//判断是否为动画
			$fp = fopen($srcfile, 'rb');
