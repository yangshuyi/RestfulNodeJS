...
function mtag_membernum_stat($start, $perpage) {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$next = true;
		$count = getcount('mtagmember', array('tagid'=>$value['tagid']));
...
function mtag_threadnum_stat($start, $perpage) {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$next = true;
		$count = getcount('mtag_thread', array('tagid'=>$value['tagid']));
...
function thread_replynum_stat($start, $perpage) {
	...
	$next = false;
	$updates = array();
	$query = $_SGLOBAL['db']->query("SELECT tid, replynum FROM ".tname('mtag_thread')." LIMIT $start,$perpage");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$next = true;
	...
	$nums = renum($updates);
	foreach ($nums[0] as $count) {
		$_SGLOBAL['db']->query("UPDATE ".tname('mtag_thread')." SET replynum=$count WHERE tid IN (".simplode($nums[1][$count]).")");
	}
	return $next;
}
