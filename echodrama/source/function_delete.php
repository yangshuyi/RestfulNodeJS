...
function deletethreads($tagid, $tids) {
	...
	$query = $_SGLOBAL['db']->query("SELECT t.* FROM ".tname('mtag_thread')." t WHERE t.tid IN(".simplode($tids).") $wheresql");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		if($allowmanage || $value['uid'] == $_SGLOBAL['supe_uid']) {
			$delthreads[] = $value;
			if(!$managebatch && $value['uid'] != $_SGLOBAL['supe_uid']) {
	...
	//删除
	$_SGLOBAL['db']->query("DELETE FROM ".tname('mtag_thread')." WHERE tid IN(".simplode($newids).")");
...
function deleteposts($tagid, $pids) {
	...
	foreach ($nums[0] as $pnum) {
		$_SGLOBAL['db']->query("UPDATE ".tname('mtag_thread')." SET replynum=replynum-$pnum WHERE tid IN (".simplode($nums[1][$pnum]).")");
	}
...
function deletespace($uid, $force=0) {
	...
	//thread
	$tids = array();
	$query = $_SGLOBAL['db']->query("SELECT tid, tagid FROM ".tname('mtag_thread')." WHERE uid='$uid'");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
...
//	}
//	$_SGLOBAL['db']->query("DELETE FROM ".tname('mtag_thread')." WHERE uid='$uid'");
//	$_SGLOBAL['db']->query("DELETE FROM ".tname('post')." WHERE uid='$uid'");
	...
	//群组
	$mtagids = array();
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('mtagmember')." WHERE uid='$uid'");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	...
	if($mtagids) {
		...
		$_SGLOBAL['db']->query("DELETE FROM ".tname('mtagmember')." WHERE uid='$uid'");
	}
...
function deletemtag($tagids) {
	...
	//数据
	$_SGLOBAL['db']->query("DELETE FROM ".tname('mtagmember')." WHERE tagid IN (".simplode($newids).")");
	$_SGLOBAL['db']->query("DELETE FROM ".tname('mtag')." WHERE tagid IN (".simplode($newids).")");
	$_SGLOBAL['db']->query("DELETE FROM ".tname('mtag_thread')." WHERE tagid IN (".simplode($newids).")");
