...
function mobile_parse_template($tpl) {
	return parse_template($tpl, MOBILE_ROOT);
}
function parse_template($tpl, $rootpath = S_ROOT) {
	global $_SGLOBAL, $_SC, $_SCONFIG;

	//包含模板
	$_SGLOBAL['sub_tpls'] = array($tpl);

	$tplfile = $rootpath.'./'.$tpl.'.htm';
	$objfile = $rootpath.'./data/tpl_cache/'.str_replace('/','_',$tpl).'.php';
	...
	if($rootpath == S_ROOT){
		...
		$template = preg_replace("/\<\!\-\-\{template\s+([a-z0-9_\/]+)\}\-\-\>/ie", "mobile_readtemplate('\\1')", $template);
	}	
	...
	//附加处理
	$template = "<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('".implode('|', $_SGLOBAL['sub_tpls'])."', '$_SGLOBAL[timestamp]', '$tpl', '".$rootpath."./"."');?>$template<?php ob_out();?>";
...
function mobile_readtemplate($name) {
	global $_SGLOBAL, $_SCONFIG;
	
	$tpl = strexists($name,'/')?$name:"template/$_SCONFIG[template]/$name";
	$tplfile = MOBILE_ROOT.'./'.$tpl.'.htm';
