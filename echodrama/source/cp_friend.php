...
if($op == 'add') {
	...
	if(!empty($_GET['searchsubmit']) || !empty($_GET['searchmode'])) {
		...
		if($searchkey = stripsearchkey($_GET['searchkey'])) {
			...
			foreach (array('uid','videostatus','avatar') as $value) {
				if($_GET[$value]) {
					$wherearr[] = "s.$value='{$_GET[$value]}'";
				}
			}
			
			foreach (array('username','name') as $value) {
				if($_GET[$value]) {
		...
		if($_GET['type'] == 'job') {
			$fromarr['resume'] = tname('resume').' r';
			$wherearr['resume'] = "s.uid=r.uid ";
			$wherearr[] = "r.jobid like '%".$_GET['jobid']."%'";
		}
		if($_GET['type'] == 'mtag') {
			$fromarr['mtagfans'] = tname('mtagfans').' mf';
			$wherearr['mtagfans'] = "mf.uid=s.uid ";
			$wherearr[] = "mf.tagid = '".$_GET['tagid']."' ";
			$wherearr[] = "mf.deleteflag = 0 ";
		}
