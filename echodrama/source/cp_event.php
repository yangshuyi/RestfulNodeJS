...
if(submitcheck('eventsubmit')) {
	...
	} elseif(empty($arr1['typeid'])){
		showmessage('event_typeid_empty');
	...
	if($arr1['typeid']!=5){
		if(empty($arr1['classid'])){
			showmessage('event_classid_empty');
		}
	}else{
		if(empty($arr1['location'])){
			showmessage('event_location_empty');
		}
	}
	...
	//默认图片
	$picPath=$_SC['attachdir'].'./'.$arr1['poster'];
	if(!is_file($picPath)){
		$picpath=getfilepath("jpg",true);
		copy("image/noeventpic.jpg",$_SC['attachdir'].'./'.$picpath); 
		$arr1['poster'] = $picpath;
		$arr1['thumb'] = 0;
		$arr1['remote'] = 0;
	}
	...
	if($_POST['tagid'] && (!$eventid || $event['uid']==$_SGLOBAL['supe_uid']) && $_POST['tagid'] != $event['tagid']) {
		...
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("mtagmember")." WHERE tagid='$_POST[tagid]' AND uid='$_SGLOBAL[supe_uid]' LIMIT 1");
		if($value=$_SGLOBAL['db']->fetch_array($query)) {
			if($value['membergradeid'] >= 1) {
...
if($op == 'invite') {
	...
	if(submitcheck('invitesubmit')){
		$invitedsql="SELECT * FROM ".tname('eventinvite')." WHERE eventid='$eventid'";
		$invitedquery = $_SGLOBAL['db']->query($invitedsql);
		$invitedarray = array();
		while ($value = $_SGLOBAL['db']->fetch_array($invitedquery)) {
			$invitedarray[$value[touid]]=1;
		}
			
		if($_POST["invitesubmittype"]==1){ //invite target
			$ids = empty($_POST['ids'])?array():$_POST['ids'];
			$names = empty($_POST['names'])?array():$_POST['names'];
		}
		elseif($_POST['invitesubmittype']==2){ //invite all friends
			$friendsql="SELECT * FROM ".tname('friend')." WHERE uid='$_SGLOBAL[supe_uid]' AND status='1'";
			$query = $_SGLOBAL['db']->query($friendsql);
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$ids[] = $value['fuid'];
				$names[] = $value['fusername'];
			}
			
			//
			getreward("event_invite_friend", 1, $_SGLOBAL[supe_uid]);
			
		}elseif($_POST['invitesubmittype']==3){ //invite all members
			//only invite 45 last login member
			$curofftime = $_SGLOBAL['timestamp'] - 2592000*3; 
	
			$friendsql="SELECT s.uid, s.username FROM ".tname('space')." s WHERE s.lastlogin>$curofftime";
			$query = $_SGLOBAL['db']->query($friendsql);
			while ($value = $_SGLOBAL['db']->fetch_array($query)) {
				$ids[] = $value['uid'];
			...
			//
			getreward("event_invite_all", 1, $_SGLOBAL[supe_uid]);
		}
		...
		for($i=0, $L=sizeof($ids); $i<$L; $i++){
			if( !empty($invitedarray[$ids[$i]]) ){
				continue;
			}
			$arr['touid'] = intval($ids[$i]);
			$arr['tousername'] = getstr($names[$i], 15, 1, 1);
	...
	if($count) {
		...
		while($value = $_SGLOBAL['db']->fetch_array($query)){
			$value['pic'] = pic_get($value['filepath'], $value['thumb'], $value['remote'],0);
			$photolist[] = $value;
		}
	}
	...
	if($count > 0) {
		...
		while($value = $_SGLOBAL['db']->fetch_array($query)){
			realname_set($value['uid'], $value['username']);
			if($value['poster']){
				$value['pic'] = pic_get($value['poster'], $value['thumb'], $value['remote'],0);
			} else {
	...
	$imagepath=pic_get($event['poster'], $value['thumb'], $value['remote'],0);
	
	addEventWaterMark($imagepath);
	...
	if(!$eventid || $event['uid']==$_SGLOBAL['supe_uid']) {
		$query = $_SGLOBAL['db']->query("SELECT mtag.* FROM ".tname("mtagmember")." st LEFT JOIN ".tname("mtag")." mtag ON st.tagid=mtag.tagid WHERE st.uid='$_SGLOBAL[supe_uid]' AND st.membergradeid>=1");
		while($value=$_SGLOBAL['db']->fetch_array($query)) {
			$mtags[] = $value;
		}
	}
