...
function mergemtag($tagids, $newtagid) {
	...
	//更新话题/回复
	$_SGLOBAL['db']->query("UPDATE ".tname('mtag_thread')." SET tagid='$newtagid' WHERE tagid IN (".simplode($tagids).")");
	$_SGLOBAL['db']->query("UPDATE ".tname('post')." SET tagid='$newtagid' WHERE tagid IN (".simplode($tagids).")");
	
	//已有的成员
	$olduids = $newuids = array();
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('mtagmember')." WHERE tagid='$newtagid'");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$olduids[$value['uid']] = $value;
	}
	
	//更新关联表
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('mtagmember')." WHERE tagid IN (".simplode($tagids).")");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		if(empty($olduids[$value['uid']])) $newuids[$value['uid']] = $value;
	}
	
	//关联
	$_SGLOBAL['db']->query("DELETE FROM ".tname('mtagmember')." WHERE tagid IN (".simplode($tagids).")");
	//插入
	$inserts = array();
	...
	if($inserts) {
		$_SGLOBAL['db']->query("REPLACE INTO ".tname('mtagmember')." (tagid,uid,username) VALUES ".implode(',', $inserts));
	}

	//更新统计
	$setarr = array(
		'membernum' => getcount('mtagmember', array('tagid'=>$newtagid)),
		'threadnum' => getcount('mtag_thread', array('tagid'=>$newtagid)),
