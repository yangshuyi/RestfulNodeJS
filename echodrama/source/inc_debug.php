...
EOF;
	...
	foreach ($_SGLOBAL['debug_query'] as $dkey => $debug) {
		...
		if($debug['info']=='CacheHint'){
			echo '<table cellspacing="0" class="'.$class.'"><tr><th rowspan="2" width="20">'.($dkey+1).'</th><td width="60" style="background:green;">'.$debug['time'].' ms</td><td class="bold" style="background:green;">'.shtmlspecialchars($debug['sql']).'</td></tr>';
		}else if($debug['time']>1){
			echo '<table cellspacing="0" class="'.$class.'"><tr><th rowspan="2" width="20">'.($dkey+1).'</th><td width="60" style="background:red;">'.$debug['time'].' ms</td><td class="bold" style="background:red;">'.shtmlspecialchars($debug['sql']).'</td></tr>';
		}else{
