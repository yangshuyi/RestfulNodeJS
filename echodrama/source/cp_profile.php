...
if(!in_array($_GET['op'], array('base','contact','edu','work','info','realname_confirm_pm', 'realname_change_pm'))) {
	$_GET['op'] = 'base';
}
...
if($_GET['op'] == 'base') {
	...
	if(submitcheck('profilesubmit') || submitcheck('nextsubmit')) {
		...
		if($setarr['name'] && strlen($setarr['name']) < 1) {//不能小于4个字符			
...
elseif ($_GET['op'] == 'realname_change_pm') {
	
	$adminusersql="SELECT s.uid FROM ".tname('space')." s WHERE s.groupid=1 ORDER BY s.uid";
	$query = $_SGLOBAL['db']->query($adminusersql);
	$list=array();
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		notification_add($value[uid],"profile_realname_change","<br/>请<a href=\"admincp.php?ac=space&op=manage&uid=$_SGLOBAL[supe_uid]\" target=\"_blank\">更改他/她的社团成员名</a><br/>$_GET[newname]",0);
	}
	showmessage('message_send_success');
}
elseif ($_GET['op'] == 'realname_confirm_pm') {
	
	$adminusersql="SELECT s.uid FROM ".tname('space')." s WHERE s.groupid=1 ORDER BY s.uid";
	$query = $_SGLOBAL['db']->query($adminusersql);
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		notification_add($value[uid],"profile_realname_confirm","<br/>请<a href=\"admincp.php?ac=space&op=manage&uid=$_SGLOBAL[supe_uid]\" target=\"_blank\">确认该社团成员名</a>",0);
	}
	showmessage('message_send_success');
}
...
$nav_item = array();
$nav_item['url'] = "cp.php?ac=profile"; $nav_item['dispname'] = "个人资料";
$nav_list[] = $nav_item;
