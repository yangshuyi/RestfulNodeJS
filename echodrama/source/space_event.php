...
if($eventid){// 显示活动内容
	...
	// 活动信息
	$qsql = "SELECT e.*, ef.* FROM ".tname("event")." e LEFT JOIN ".tname("eventfield")." ef ON e.eventid=ef.eventid WHERE e.eventid='$eventid'";
	$event = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, true, 86400);
	if(empty($event)){
	...
	if($view == "member"){
		...
		if($event['poster']){
			...
			//TODO do not use the default event class type
			$event['pic'] = 'image/event/default.jpg';
		}
...
//		$threadlist = array();
//		if($event['tagid']) {
//			$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('mtag_thread')." WHERE eventid='$eventid'"),0);
//			if($count) {
//				$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('mtag_thread')." WHERE eventid='$eventid' ORDER BY lastpost DESC LIMIT 10");
//				while ($value = $_SGLOBAL['db']->fetch_array($query)) {
//					realname_set($value['uid'], $value['username']);
//					$threadlist[] = $value;
//				}
...
//		}
	...
	if($view == "all"){
		// 只在全部活动下显示
		$qsql = "SELECT * FROM ".tname("event")." WHERE grade = 2 ORDER BY jointype DESC, recommendtime DESC LIMIT 4";
		$resultlist = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, false, 3600);
	...
	$hotevents = array();
	if($view == 'friend') {
		$curofftime = getCurrentDate(); 
		$qsql = 'SELECT * FROM '.tname('event')." WHERE endtime > '$curofftime' ORDER BY jointype DESC, membernum ASC LIMIT 6";
		$resultlist = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, false, 3600);
		foreach($resultlist as $value){
			//招募分类
			$classname="";
	...
	if($space['feedfriend'] && $view != "friend" && $view != "me"){
		$query = $_SGLOBAL['db']->query("SELECT ue.*, e.*, ue.uid as fuid, ue.username as fusername FROM ".tname("userevent")." ue LEFT JOIN ".tname("event")." e ON ue.eventid=e.eventid WHERE ue.uid IN ($space[feedfriend]) AND ue.status >= 2 ORDER BY e.jointype DESC, ue.dateline DESC LIMIT 6");
		while($value = $_SGLOBAL['db']->fetch_array($query)){
	...
	if($view != "me"){
		// 在我的活动标签下不显示
		$query = $_SGLOBAL['db']->query("SELECT ue.*, e.* FROM ".tname("userevent")." ue LEFT JOIN ".tname("event")." e ON ue.eventid=e.eventid WHERE ue.uid = '$_SGLOBAL[supe_uid]' AND ue.status = 1 ORDER BY e.jointype DESC, ue.dateline LIMIT 6");
		while($value = $_SGLOBAL['db']->fetch_array($query)){
			$followevents[] = $value;
			realname_set($value['uid'], $value['username']);
		}
	...
	if($view=="recommend") {
		...
		if($type=="admin"){
			$wherearr[] = "e.grade = 2";
			$orderby = " e.jointype DESC, e.recommendtime DESC";
			$theurl .= "&type=admin";
		} else {
			$wherearr[] = "e.endtime > '$_SGLOBAL[timestamp]'";
			$orderby = " e.jointype DESC, e.membernum DESC";
			$theurl .= "&type=hot";
		}
	} elseif($view=="city") {
		$fromsql = tname("event")." e";
		if($type=="over") {
			$wherearr[] = "e.endtime < '$_SGLOBAL[timestamp]'";
			$orderby = " e.jointype DESC, e.eventid DESC";
			$theurl .= "&type=over";
		} else {
			$wherearr[] = "e.endtime >= '$_SGLOBAL[timestamp]'";
			$orderby = " e.jointype DESC, e.eventid DESC";
		...
		if($count) {
			...
			$orderby = "e.jointype DESC,  e.eventid DESC";
			$sql = "SELECT e.*, ue.uid as fuid, ue.username as fusername, ue.status FROM $fromsql $joinsql WHERE ".implode(" AND ", $wherearr) ." ORDER BY $orderby";
		}
		//showmessage($sql);
		$needquery = false;
		...
		$joinsql = "";
		$orderby = "e.jointype DESC, ue.dateline DESC";
		...
		if($type=="follow") {
			...
			$sql = "SELECT * FROM ".tname('event')." e WHERE e.uid='$space[uid]' ORDER BY e.jointype DESC, e.dateline DESC LIMIT $start, $perpage";
		}
	...
	//
	if($view == 'city') {
		$wherearr[] = "e.typeid = ".$_SGLOBAL[eventtype][5][classid];
	}
	else{
		$wherearr[] = "e.typeid <> ".$_SGLOBAL[eventtype][5][classid];
	}
	...
	if($_GET['typeid']){
		...
		$wherearr[] = "e.typeid = $_GET[typeid]";
		$theurl .= "&typeid=$_GET[typeid]";
		$typeid=$_GET['typeid'];
	}
	...
	if($_GET['classid']){
		...
		$wherearr[] = "e.classid like '%,$_GET[classid],%'";
	...
	if($needquery) {// 好友的活动已特别处理
		$sql = "SELECT COUNT(*) AS count FROM $fromsql WHERE ".implode(" AND ", $wherearr);
		$count = $_SGLOBAL['db']->cachequery($MODULE_NAME, $sql, true, $page == 1?60:1800);
		$count = $count['count'];
	}
	...
	if($count){
		...
		$resultlist = $_SGLOBAL['db']->cachequery($MODULE_NAME, $sql, false, $page == 1?60:1800);
