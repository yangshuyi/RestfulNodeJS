...
if($_GET['view'] == 'all') {
	...
	$wheresql = "1";//没有隐私
	$ordersql = "feed.dateline DESC";
	$theurl = "space.php?uid=$space[uid]&do=$do&view=all";
	$f_index = '';

} elseif($_GET['view'] == 'hot') {

	$wheresql = "feed.hot>='$minhot'";
	$ordersql = "feed.dateline DESC";
	...
	if( $_GET['view'] == 'me') {
		$wheresql = "feed.uid='$space[uid]'";
		$ordersql = "feed.dateline DESC";
		$theurl = "space.php?uid=$space[uid]&do=$do&view=me";
		$f_index = '';
		
	} else {
		$wheresql = "feed.uid IN ('0',$space[feedfriend])";
		$ordersql = "feed.dateline DESC";
...
$count = $filtercount = 0;
$qsql="SELECT feed.*, ";
$qsql.="     (topic.poll_1*1+topic.poll_2*2+topic.poll_3*3+topic.poll_4*4+topic.poll_5*5) AS topicpollsum, ";
$qsql.="     (tone.poll_1*1+tone.poll_2*2+tone.poll_3*3+tone.poll_4*4+tone.poll_5*5) AS tonepollsum ";
$qsql.="   FROM ".tname('feed')." feed $f_index";
$qsql.=" LEFT OUTER JOIN ".tname('topic')." topic ON feed.id=topic.topicid AND feed.idtype='topicid' ";
$qsql.=" LEFT OUTER JOIN ".tname('tone')." tone ON feed.id=tone.toneid AND feed.idtype='toneid' ";
$qsql.=" WHERE $wheresql ORDER BY $ordersql LIMIT $start,$perpage";

if($_GET['view'] == 'me' || $_GET['view'] == 'hot') {
	//个人动态
	$recordlist = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, false, 1800);
...
if($space['self'] && empty($start)) {
	...
	//获取任务
	//include_once(S_ROOT.'./source/function_announcement.php');
	//$announcement = loadtodayannouncement();
	...
	if($_SCONFIG['feedhotnum'] > 0 && ($_GET['view'] == 'we' || $_GET['view'] == 'all')) {
		$hotlist_all = array();
		$curofftime = getCurrentDate() - $_SCONFIG['feedhotday']*86400; 
		$qsql = "SELECT * FROM ".tname('feed')." USE INDEX(hot) WHERE dateline>='$curofftime' ORDER BY hot DESC LIMIT 0,10";
		$resultlist = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, false, 86400);
	...
	//广播剧
	$qsql = "SELECT * FROM ".tname('topic')." where type='single' and auditstate=2 ORDER BY lastpost DESC LIMIT 0,1";
	$newtopic = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, true, 1800);
	$newtopic['picthumb'] = $_SC['attachdir'].$newtopic['pic'].'.thumb.jpg';
	$topiclist[] = $newtopic;
	
	//声线
	$qsql = "SELECT * FROM ".tname('tone')." ORDER BY lastpost DESC LIMIT 0,1";
	$newtone = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, true, 1800);
	$tonelist[] = $newtone;
...
//页面模块
$site_module_actives = array('space_home' => ' class="portalmenuli-sel"');
