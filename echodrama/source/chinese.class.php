 ...
 * 简体中文、繁体中文 <-> UTF-8 编码转换，简体中文、繁体中文 -> Unicode单向转换
 *
 ...
 * 增加 简体中文转换为 UTF-8 的功能。
 * 增加 繁体中文转换为 UTF-8 的功能。
 *
 * ver 1.0 2003-04-01
...
class Chinese
{
	 ...
	 */
	var $config  =  array(
		'codetable_dir'         => "./class/tables/",           //  存放各种语言互换表的目录
		'SourceLang'            => '',                    //  字符的原编码
		'TargetLang'            => '',                    //  转换后的编码
	...
	function Chinese( $SourceLang , $TargetLang , $SourceString='', $CodetableDir='')
	{
		...
		if(!function_exists('iconv')) {
			$SourceLang = str_replace('GBK', 'GB2312', $SourceLang);
			$TargetLang = str_replace('GBK', 'GB2312', $TargetLang);
		}
		...
		if(function_exists('iconv')) {
			...
			if ($CodetableDir != '') {
				...
				$CodetableDir = S_ROOT.'./class/tables/';
				$this->config['codetable_dir'] = $CodetableDir;
			}
	...
	function OpenTable()
	{
		...
		if ($this->config['SourceLang']=="GB2312") {
			...
			if ($this->config['TargetLang'] == "BIG5") {
				...
				if (is_null($this->ctf)) {
					echo "GBtoBIG5_table not exists";
					exit;
				}
			}
			...
			if ($this->config['TargetLang'] == "PinYin") {
				...
				if (!$tmp) {
					echo "GBtoPinYin_table not exists";
					exit;
				}
				//
				$i = 0;
			...
			if ($this->config['TargetLang'] == "UTF-8") {
				...
				if (!$tmp) {
					echo "GBtoUnicode_table not exists";
					exit;
				}
		...
		if ($this->config['SourceLang']=="BIG5") {
			...
			if ($this->config['TargetLang'] == "GB2312") {
				...
				if (is_null($this->ctf)) {
					echo "BIG5toGB_table not exists";
					exit;
				}
			}
			// 假如转换目标编码为 UTF-8 的话
			if ($this->config['TargetLang'] == "UTF-8") {
				...
				if (!$tmp) {
					echo "BIG5toUnicode_table not exists";
					exit;
				}
				$this->unicode_table = array();
				while(list($key,$value)=each($tmp))
...
//		ereg(href="css/dir.css"
	...
	/**
	 * 将简体、繁体中文的 UNICODE 编码转换为 UTF-8 字符
	 *
	 * 详细说明
	...
	function ConvertIT()
	{
		...
		if($this->iconv_enabled) {
			if($this->config['SourceLang']=='UTF-8' && preg_match("/^GB/i", $this->config['TargetLang'])) {
				//将utf-8中的 '·'、'—'、'' 替换成 '?'、'―'、'　'
				$searchiconv = array(chr(194).chr(183), chr(226).chr(128).chr(148), chr(238).chr(151).chr(164));
				$replaceiconv = array(chr(227).chr(131).chr(187), chr(226).chr(128).chr(149), chr(227).chr(128).chr(128));
				$this->SourceText = str_replace($searchiconv, $replaceiconv, $this->SourceText);
			}
			return iconv($this->config['SourceLang'], $this->config['TargetLang'], $this->SourceText);
		}
		...
		// 判断是否为简体、繁体中文与UTF8转换
		if ( ($this->config['SourceLang']=="GB2312" || $this->config['SourceLang']=="BIG5" || $this->config['SourceLang']=="UTF-8") && ($this->config['TargetLang']=="UTF-8" || $this->config['TargetLang']=="GB2312" || $this->config['TargetLang']=="BIG5") ) {
