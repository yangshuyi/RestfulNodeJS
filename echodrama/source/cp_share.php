...
if(!empty($_GET['special'])){
	...
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('share')." WHERE type='topic' and id is null ORDER BY dateline DESC ");
	$list = array();
	...
	foreach($list as $value) {
		$topicid = $value['body_data']['topicid'];
		$query = $_SGLOBAL['db']->query("UPDATE ".tname("share")." SET id = '$topicid' WHERE sid='$value[sid]'");
	}

	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('share')." WHERE type='tone' and (id is null || id=0) ORDER BY dateline DESC ");
	$list = array();
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		realname_set($value['uid'], $value['username']);
		$value = mkshare($value);
	...
	foreach($list as $value) {
		$toneid = $value['body_data']['toneid'];
//		showmessage("UPDATE ".tname("share")." SET id = '$toneid' WHERE sid='$value[sid]'");
		$query = $_SGLOBAL['db']->query("UPDATE ".tname("share")." SET id = '$toneid' WHERE sid='$value[sid]'");
	}
	
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('share')." WHERE type='cover' and (id is null || id=0) ORDER BY dateline DESC ");
	$list = array();
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		realname_set($value['uid'], $value['username']);
		$value = mkshare($value);
	...
	foreach($list as $value) {
		$coverid = $value['body_data']['coverid'];
		$query = $_SGLOBAL['db']->query("UPDATE ".tname("share")." SET id = '$coverid' WHERE sid='$value[sid]'");
	}
	
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('share')." WHERE type='video' and (id is null || id=0) ORDER BY dateline DESC ");
	$list = array();
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		realname_set($value['uid'], $value['username']);
		$value = mkshare($value);
	...
	foreach($list as $value) {
		$videoid = $value['body_data']['videoid'];
		$query = $_SGLOBAL['db']->query("UPDATE ".tname("share")." SET id = '$videoid' WHERE sid='$value[sid]'");
	}
...
if($_GET['op'] == 'delete') {
	...
	switch ($type) {
		...
		case 'pic':
			...
			break;
		case 'thread':
			$query = $_SGLOBAL['db']->query("SELECT t.*, p.message, p.hotuser FROM ".tname('mtag_thread')." t
			...
			$arr['body_data'] = array(
				'subject' => "<a href=\"space.php?do=thread&tid=$thread[tid]\">$thread[subject]</a>",
			...
			//通知
			$note_uid = $thread['uid'];
			$note_message = cplang('note_share_thread', array("space.php?do=thread&tid=$thread[tid]", $thread['subject']));
		...
		case 'event':
			...
			if(empty($event['poster'])){
			    $arr['image'] = 'image/event/default.jpg';
			}
		...
		case 'topic':
			...
			//no need check for uid
//			if($topic['uid'] == $space['uid']) {
//				showmessage('share_not_self');
//			}
//			
//			//黑名单
//			if(isblacklist($topic['uid'])) {
//				showmessage('is_blacklist');
//			}
		    $arr['title_template'] = cplang('share_topic');
			$arr['body_template'] = '<b>[{typename}]<a href="space.php?do=topic&topicid={topicid}">{subject}</a></b><br>类型：{productclassname}<br>社团：{club}';
			$arr['body_data'] = array(
			...
			$arr['image'] = $topic['picthumb'];
			$arr['image_link'] = "space.php?do=topic&topicid=$topic[topicid]";
			break;
		case 'cover':
			...
			$arr['title_template'] = cplang('share_cover');
			$arr['body_template'] = '<b>[{typename}]<a href="space.php?do=cover&coverid={coverid}">{subject}</a></b><br>类型：{productclassname}<br>翻唱：{singer}';
			$arr['body_data'] = array(
				'subject' => $cover['subject'],
				'singer' => $cover['singer'],
				'coverid' => $cover['coverid'],
				'productclassname' => $cover['productclassname'],
				'typename' => '乐虫翻唱'
			);
			$arr['image'] = null;
			$arr['image_link'] = null;
		...
		case 'video':
			...
			$arr['body_template'] = '<b>[{typename}]<a href="space.php?do=video&videoid={videoid}">{subject}</a></b><br>类型：{productclassname}<br>社团：{club}';
			$arr['body_data'] = array(
				'subject' => $video['subject'],
				'club' => $video['club'],
				'videoid' => $video['videoid'],
				'productclassname' => $video['productclassname'],
				'typename' => '视频映像'
			);
			$arr['image'] = null;
			$arr['image_link'] = null;
			break;
		case 'tone':
		    $query = $_SGLOBAL['db']->query("SELECT t.*	FROM ".tname("tone")." t WHERE t.toneid='$id'");
		    if(!$tone = $_SGLOBAL['db']->fetch_array($query)){
		        showmessage('tone_does_not_exist');
		    }		    
			if($tone['uid'] == $space['uid']) {
				showmessage('share_not_self');
			}
			
			//黑名单
			if(isblacklist($tone['uid'])) {
				showmessage('is_blacklist');
			}

		    $arr['title_template'] = cplang('share_tone');
			$arr['body_template'] = '<b><a href="space.php?do=tone&toneid={toneid}">{label}</a></b><br>';
			$arr['body_data'] = array(
				'label' => $tone['label'],
				'toneid' => $tone[toneid]
			);
		...
		case 'poll':
			...
			break;
		case 'repaste':
			$query = $_SGLOBAL['db']->query("SELECT r.*,rf.message,rf.hotuser FROM ".tname('repaste')." r
				LEFT JOIN ".tname('repastefield')." rf ON rf.repasteid=r.repasteid
				WHERE r.repasteid='$id'");
			if(!$repaste = $_SGLOBAL['db']->fetch_array($query)) {
				showmessage('repaste_does_not_exist');
			}
			if($repaste['uid'] == $space['uid']) {
				showmessage('share_not_self');
			}

			//黑名单
			if(isblacklist($repaste['uid'])) {
				showmessage('is_blacklist');
			}

			//实名
			realname_set($repaste['uid'], $repaste['username']);
			realname_get();

			$arr['title_template'] = cplang('share_repaste');
			$arr['body_template'] = '<b>{subject}</b><br>{username}<br>{message}';
			$arr['body_data'] = array(
				'subject' => "<a href=\"space.php?uid=$repaste[uid]&do=repaste&id=$repaste[repasteid]\">$repaste[subject]</a>",
				'username' => "<a href=\"space.php?uid=$repaste[uid]\">".$_SN[$repaste['uid']]."</a>",
				'message' => getstr($repaste['message'], 150, 0, 1, 0, 0, -1)
			);
			//通知
			$note_uid = $repaste['uid'];
			$note_message = cplang('note_share_repaste', array("space.php?uid=$repaste[uid]&do=repaste&id=$repaste[repasteid]", $repaste['subject']));
...
//		$_POST['topicid'] = topic_check($_POST['topicid'], 'share');
		...
		if( strlen($_POST['general']) < 50 ) {
			showmessage( 'share_message_limit' );
		}
		
		$arr['body_general'] = getstr($_POST['general'], 8000, 1, 1, 1, 1);
		...
		$notificationmessage = "<a href='space.php?uid=$_SGLOBAL[supe_uid]&do=share&id=$sid' target='_blank'>".$arr['title_template']."</a>";
		
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('share')." WHERE sid = $sid ");
		$value = $_SGLOBAL['db']->fetch_array($query);
		if( !empty( $value ) ) {
			$value = mkshare($value);
			$notificationmessage.= '<br/>'.$value['body_template'];
		}
		
		$notificationmessage.='<br/>分享说明：<br/>'.getstr($_POST['general'], 60, 1, 1, 1, 1);
		
		$friendsql="SELECT * FROM ".tname('friend')." WHERE uid='$_SGLOBAL[supe_uid]' AND status='1'";
		$query = $_SGLOBAL['db']->query($friendsql);
		while ($friend = $_SGLOBAL['db']->fetch_array($query)) {
			if($friend['fuid'] == $note_uid){
				continue;
			}
			notification_add($friend['fuid'], 'sharenotice', $notificationmessage);
		}
		...
		savecomment($cid, $idtype, $id, $message, 0, 0);
