...
switch ($idtype) {
	case 'picid':
		...
		break;
	case 'tid':
		$sql = "SELECT t.*, p.hotuser FROM ".tname('mtag_thread')." t
			LEFT JOIN ".tname('post')." p ON p.tid='$id' AND p.isthread='1'
			WHERE t.tid='$id'";
		$tablename = tname('mtag_thread');
		break;
	default:
		$idtype = 'blogid';
...
if($_GET['op'] == 'add') {
	...
	switch ($idtype) {
		...
		case 'tid':
			...
			$fs['title_data'] = array(
				'touser' => "<a href=\"space.php?uid=$item[uid]\">{$_SN[$item['uid']]}</a>",
				'subject' => "<a href=\"space.php?do=thread&tid=$item[tid]\">$item[subject]</a>",
				'click' => $click['name']
			);
			$note_type = 'clickthread';
			$q_note = cplang('note_click_thread', array("space.php?do=thread&tid=$item[tid]", $item['subject']));
			break;
		case 'picid':
