...
//得到海报合适的大小
function getsuitablewidth($picfile, $maxwidth){
...
function buildlabel($labellist, $minweight, $maxweight, $totalweight, $expendbasenumber){
	global $_SGLOBAL, $_SC;
	include_once(S_ROOT.'./data/data_labelcolorclass.php');
	
	$newlabellist = array();
	foreach($labellist as $item){
		//fontsize
		if( ($item['labelweight'] / $totalweight) >= 0.4){
			$item['fontsize'] = 18;
		}elseif( ($item['labelweight'] / $totalweight) >= 0.3){
			$item['fontsize'] = 14;
		}elseif( ($item['labelweight'] / $totalweight) >= 0.1){
			$item['fontsize'] = 12;
		}else{
			$item['fontsize'] = 12;
		}
		...
		//color
		$colorIdx = rand(1,10);
		$item['color'] = $_SGLOBAL['labelcolorclass'][$colorIdx]['color'];
		$item['backgound'] = $_SGLOBAL['labelcolorclass'][$colorIdx]['backgound'];
		//radius
		$baseweight_absolute = sqrt(abs( $item['labelweight'] - $maxweight ));
		$baseweight_relative = sqrt(( $totalweight / $item['labelweight'] ));
		
		$itemweight = ($baseweight_absolute + $baseweight_relative)*12/13 ;
		$item['radius'] = $itemweight;
...
//标签生成
function buildLabelDisplay($modulename, $labellist, $width, $height){
	$html_str = "<div id=\"label_display_div_container_".$modulename."\" style=\"position:relative; z-index=99; width:{$width}px; height:{$height}px; overflow:hidden;\">";
	$index = 0;
	foreach($labellist as $item){
//		$phi = acos(-1+(2*($index+1)-1)/count($labellist));
//		$theta = sqrt( (count($labellist)+1)*3.141592654 )*$phi;
		
		$deliver = (2*3.14)/count($labellist);
		$theta = (-1+($index*$deliver));
		//坐标变换
		$cx = $item['radius'] *20* cos($theta);
		$cy = $item['radius'] *20* sin($theta);
		
		$offsetx=$cx+$width/2-20+'px';
		$offsety=$cy+$height/2-20+'px';
		
//		$fileopen = fopen("c:/c.txt","a+");
//		fwrite($fileopen,"count:".count($labellist)."deliver:".$deliver."ta:".$theta."cx:".$cx."cy:".$cy."offsetx:".$offsetx."offsety:".$offsety."\n");
//		fclose($fileopen);
		
		$index=$index+1;
		
		$html_item_str = "<div id=\"label_display_div_item_".$modulename."\" ";
		$html_item_str.= "style=\"position:absolute;padding:2px;font-weight: bold;font-size:{$item[fontsize]}px;color:{$item[color]};top:{$offsety}px;left:{$offsetx}px\" ";
		$html_item_str.= " title=\"{$item[label]} - 次数： {$item[labelweight]}\"";
		$html_item_str.= " onclick=\"javascript:labeldisplaydivitem_onclick(this);\" ";
		$html_item_str.= " onMouseOver=\"this.style.cursor='pointer';\" ";
		$html_item_str.= ">";
		$html_item_str.= $item[label];
		$html_item_str.= "</div>";
		$html_str.=$html_item_str;
	}
	
	$html_str.= "<script type=\"text/javascript\">";
	$html_str.= "function labeldisplaydivitem_onclick(divItem){";
	$html_str.= "}";
	$html_str.= "";
	$html_str.= "</script>";
	$html_str.= "</div>";
...
function dbconnect() {
	...
	if(empty($_SGLOBAL['db'])) {
		...
		$_SGLOBAL['db']->connect($_SC['dbhost'], $_SC['dbuser'], $_SC['dbpw'], $_SC['dbname'], $_SC['pconnect'], $_SC['memcacheSetting']);
	}
	
	if(empty($_SGLOBAL['consoledb'])) {
		$_SGLOBAL['consoledb'] = new dbstuff;
		$_SGLOBAL['consoledb']->charset = $_SC['dbcharset'];
		$_SGLOBAL['consoledb']->connect($_SC['console_dbhost'], $_SC['console_dbuser'], $_SC['console_dbpw'], $_SC['console_dbname'], $_SC['console_pconnect'], $_SC['memcacheSetting']);
	}
}
...
//获取到CONSOLE表名
function tnameconsole($name) {
	global $_SC;
	return $_SC['console_tablepre'].$name;
}

function gethelplink($helplinkkey) {
	//语言
	include_once(S_ROOT.'./data/data_helplink.php');
	$link = $_SGLOBAL['helplink'][$helplinkkey];
	return "<div style=\"width:100px; height:50px; background: transparent url(/image/helpbg.jpg) no-repeat left top; \"><a href='$link' target='_blank'>帮助</a>";
}

function loadmessage($msgkey, $values=array()) {
	//语言
...
//对话框
function mobile_showmessage($msgkey, $url_forward='', $second=1, $values=array()) {
	...
	if(empty($_SGLOBAL['inajax']) && $url_forward && empty($second)) {
		...
		if($_SGLOBAL['inajax']) {
			...
			include mobile_template('showmessage');
		}
	}
	exit();
}
...
//添加数据
function inserttablebbs($tablename, $insertsqlarr, $returnid=0, $replace = false, $silent=0) {
	global $_SGLOBAL;
	...
	$method = $replace?'REPLACE':'INSERT';
	$sql=$method.' INTO '.tnamebbs($tablename).' ('.$insertkeysql.') VALUES ('.$insertvaluesql.')';
//	showmessage($sql);
	$_SGLOBAL['bbsdb']->query($sql, $silent?'SILENT':'');
	if($returnid && !$replace) {
		return $_SGLOBAL['bbsdb']->insert_id();
	}
}
...
function reloadspace($uid){
	...
	updatetable('session', array('reload'=>1), array('uid'=>$uid));
}
...
function getspace($key, $indextype='uid', $auto_open=0) {
	...
	if(empty($_SGLOBAL[$var])) {
		$reload = false;
		if(!empty($_SGLOBAL['supe_uid'])){
			$qsql="SELECT s.reload AS reload FROM ".tname("session")." s WHERE s.uid='$_SGLOBAL[supe_uid]'";
			$query = $_SGLOBAL['db']->query($qsql);
			$session = $_SGLOBAL['db']->fetch_array($query);
			if($session['reload']==1){
				$reload = true;
				$query = $_SGLOBAL['db']->query("UPDATE ".tname('session')." SET reload=0 WHERE uid='$_SGLOBAL[supe_uid]'");
			}
		}
		
		$space = array();
		$qsql="SELECT sf.*, s.*, ug.system FROM ".tname('space')." s ";
		$qsql.=" LEFT JOIN ".tname('spacefield')." sf ON sf.uid=s.uid ";
		$qsql.=" LEFT JOIN ".tname('usergroup')." ug ON ug.gid=s.groupid ";
		$qsql.=" WHERE s.{$indextype}='$key'";
		
		$space = $_SGLOBAL['db']->cachequery($MODULE_NAME, $qsql, true, 900, $reload);
		if(empty($space)) {
			$space = array();
...
function queryuserbyid($uid){
	...
	$qsql = "SELECT s.*, r.club, r.jobid FROM ".tname('space')." s ";
	$qsql .= "left outer join ".tname('resume')." r on s.uid=r.uid WHERE s.uid='$uid'";
...
function getSystemCache(){
	...
	$system_serializer_modulename = "SystemCache";
	...
	if( !empty($system_serializer_array) ){
		//虫虫小喇叭
		$returnarray['site_announcement_thread_topitem'] = $system_serializer_array['site_announcement_thread_topitem'];
		//站点管理员信息
		$returnarray['site_administrator_list'] = $system_serializer_array['site_administrator_list'];
	}
	else{
		//虫虫小喇叭
		$querysql="SELECT * FROM ".tname('mtag_thread')." mt WHERE mt.close = 0 and mt.tagid = 8 and mt.threadclassid= 40 ORDER BY mt.dateline DESC LIMIT 0,1";
		$query = $_SGLOBAL['db']->query($querysql);
		$site_announcement_thread_topitem = $_SGLOBAL['db']->fetch_array($query);
		$site_announcement_thread_topitem['datelinedisp'] = sgmdate('Y-m-d', $site_announcement_thread_topitem['dateline']);
		
		$system_serializer_array['site_announcement_thread_topitem'] = $site_announcement_thread_topitem;
		$returnarray['site_announcement_thread_topitem'] = $system_serializer_array['site_announcement_thread_topitem'];
		
		//站点管理员信息
		$site_administrator_list = array();
		$query = $_SGLOBAL['db']->query("select * from ".tname('mtagmember')." where tagid = 8 ");
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			$site_administrator_list[] = $value;
		}
		
		$system_serializer_array['site_administrator_list'] = $site_administrator_list;
		$returnarray['site_administrator_list'] = $system_serializer_array['site_administrator_list'];
	...
	//站点管理员信息 - 用户确认
	if( !empty($_SGLOBAL[supe_uid]) ) {
		foreach($returnarray['site_administrator_list'] as $value){
			if( $value[uid] == $_SGLOBAL[supe_uid] ){
				$_SGLOBAL['admin'] = 1;
				if( $value[membergradeid] >= 8) {
						$_SGLOBAL['administrator'] = 1;
				}
				//superyang_xp / 九月 / 卡特 / 爱爱 / 星庭 / 伝 /  
				if ( ( $_SGLOBAL['supe_uid'] == 1 ) || ( $_SGLOBAL['supe_uid'] == 5846 ) || ( $_SGLOBAL['supe_uid'] == 16243 ) || ( $_SGLOBAL['supe_uid'] == 12819 ) || ( $_SGLOBAL['supe_uid'] == 4286 ) || ( $_SGLOBAL['supe_uid'] == 214 ) ){
					$_SGLOBAL['sns_admin'] = 1;
				}
			}
		}
		
		//superyang_xp / 九月 / 卡特 / Gentle / 爱天使的海豚 / 茶几小水
		if ( ( $_SGLOBAL['supe_uid'] == 2 ) || ( $_SGLOBAL['supe_uid'] == 5846 ) || ( $_SGLOBAL['supe_uid'] == 16243 ) || ( $_SGLOBAL['supe_uid'] == 19943 ) || ( $_SGLOBAL['supe_uid'] == 20312 ) || ( $_SGLOBAL['supe_uid'] == 3076 ) ){
			$_SGLOBAL['forum_admin'] = 1;
			$_SGLOBAL['forum_admin_FORUM_JISHUJIAOLIU_CLASSID'] = 1;
			$_SGLOBAL['forum_admin_FORUM_ZIYUANHUZHU_CLASSID'] = 1;
			$_SGLOBAL['forum_admin_FORUM_XILIJUPING_CLASSID'] = 1;
			$_SGLOBAL['forum_admin_FORUM_QIZUIBASHE_CLASSID'] = 1;
		}
		
		//七嘴八舌 - 茶几小水 / 醉难笑
		if ( ( $_SGLOBAL['supe_uid'] == 16150 ) ){
			$_SGLOBAL['forum_admin'] = 1;
			$_SGLOBAL['forum_admin_FORUM_QIZUIBASHE_CLASSID'] = 1;
		}
		
		//技术交流 - 小乐
		if ( ( $_SGLOBAL['supe_uid'] == 22092 ) ){
			$_SGLOBAL['forum_admin'] = 1;
			$_SGLOBAL['forum_admin_FORUM_JISHUJIAOLIU_CLASSID'] = 1;
		}
		
		//犀利评论 - 玖骨画蝎 / 小色
		if ( ( $_SGLOBAL['supe_uid'] == 21960 ) || ( $_SGLOBAL['supe_uid'] == 10791 ) ){
			$_SGLOBAL['forum_admin'] = 1;
			$_SGLOBAL['forum_admin_FORUM_XILIJUPING_CLASSID'] = 1;
		}
		
		//资源互助 - 恩妮蜗牛
		if ( ( $_SGLOBAL['supe_uid'] == 20249 ) || ( $_SGLOBAL['supe_uid'] == 1 ) ){
			$_SGLOBAL['forum_admin'] = 1;
			$_SGLOBAL['forum_admin_FORUM_ZIYUANHUZHU_CLASSID'] = 1;
		}
	}
...
function getCurrentWeek(){
	return sgmdate('YmW', $_SGLOBAL['timestamp']);
}

function getCurrentDate(){
	$year = intval(sgmdate('Y', $_SGLOBAL['timestamp']));
	$month = intval(sgmdate('m', $_SGLOBAL['timestamp']));
	$day = intval(sgmdate('d', $_SGLOBAL['timestamp']));
	
	$date = mktime(0,0,0,$month,$day,$year);
	//should be convert twice for local timezone
	$datestr = sgmdate('Y-m-d', $date);
	return sstrtotime($datestr);
}
...
function mobile_template($name) {
	...
	$objfile = MOBILE_ROOT.'./data/tpl_cache/'.str_replace('/','_',$tpl).'.php';
	
	if(!file_exists($objfile)) {
		include_once(S_ROOT.'./source/function_template.php');
		mobile_parse_template($tpl);
	}
	return $objfile;
}
...
//获取文件名后缀
function fileextwithcase($filename) {
...
//		$fileopen = fopen("d:/login.txt","a+");
//		fwrite($fileopen,$ucresult[0]." - ".$username." - ".$password."\n");
//		fclose($fileopen);
...
//检查是否登录
function mobile_checklogin() {
	global $_SGLOBAL, $_SCONFIG;

	if(empty($_SGLOBAL['supe_uid'])) {
		ssetcookie('_refer', rawurlencode($_SERVER['REQUEST_URI']));
		mobile_showmessage('to_login', 'do.php?ac='.$_SCONFIG['login_action']);
	}
}
...
function loadnamemap($uidarray){
	global $_SGLOBAL;
	$namearray = array();
	
	$query = $_SGLOBAL['db']->query("SELECT uid, username FROM ".tname('space')." WHERE uid IN (".simplode($uidarray).")");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$namearray[$value['uid']] = $value['username'];
	}
	return $namearray;
}

function loadrealnamemap($uidarray){
	global $_SGLOBAL;
	$realnamearray = array();
	
	$query = $_SGLOBAL['db']->query("SELECT uid, username, name, namestatus FROM ".tname('space')." WHERE uid IN (".simplode($uidarray).")");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		if(empty($value['name'])){
			$realnamearray[$value['uid']] = $value['username'];
		}
		
		if($value['namestatus'] != 1){
			$realnamearray[$value['uid']] = $value['username'];
		}
		$realnamearray[$value['uid']] = $value['name'];
	}
...
function getmtag($id) {
	...
	$query = $_SGLOBAL['db']->query("SELECT membergradeid FROM ".tname('mtagmember')." WHERE tagid='$id' AND uid='$_SGLOBAL[supe_uid]' LIMIT 1");
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
...
function queryusergroup($uid) {
	...
	$LOCAL_MODULE_NAME = "usergroup";
	
	$qsql = "SELECT s.groupid AS groupid FROM ".tname('space')." s WHERE s.uid='$uid'";

	$groupid = $_SGLOBAL['db']->cachequery($LOCAL_MODULE_NAME, $qsql, true, 86400);
...
function g_iconbyuid($uid) {
	global $_SGLOBAL;
	
	$gid = queryusergroup($uid);
...
function g_namebyuid($uid) {
	...
	echo $_SGLOBAL[grouptitle][$gid][grouptitle];
}

//将face表情转换为安全的图标
function face_safe($content){

	$pattern="/<img src=\"(image\/face\/\d+.gif\") (class=\"face\")>/";
...
function socket_open($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
	$return = '';
