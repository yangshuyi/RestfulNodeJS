...
function updatemtagcredit($tagid, $uid, $username, $action, $credit){
	global $_SGLOBAL,$_SC;
...
function mp3_tempnamegen($originname) {
	global $_SGLOBAL, $_SCONFIG, $space, $_SC;
	
	$uploadDir = S_ROOT. '/tempfile/';
	$originname=iconv("utf-8","gbk",$originname);
	$filename = $uploadDir. $originname;
	if(is_file($filename)){
		unlink($filename);
	}
	return $filename;
...
function mp3_save($originname,$type) {
	global $_SGLOBAL, $_SCONFIG, $space, $_SC;

	$uploadDir = S_ROOT. '/tempfile/';
	
	if(is_file($uploadDir.$originname)){
		$attachmentDir = $_SC['attachdir'];
		
		$newfilename='./';
		if('topic'==$type){
			$newfilename = $newfilename.'topic/';
		}elseif('tone'==$type){
			$newfilename = $newfilename.'tone/';
		}elseif('radio'==$type){
			$newfilename = $newfilename.'radio/';
		}else{
			$newfilename = $newfilename.'other/';
		}
		$newfilename = $newfilename.random(4).'-'.$_SGLOBAL['timestamp'].".mp3";
		copy($uploadDir.$originname, $attachmentDir.$newfilename); //拷贝到新目录
		unlink($uploadDir.$originname); //删除旧目录下的文件
		return $newfilename;
	}
	return null;
}

function topic_attachment_save($originname) {
	global $_SGLOBAL, $_SCONFIG, $space, $_SC;

	$uploadDir = S_ROOT. '/tempfile/';
	
	$suffix=end(explode(".",$originname)); 
	
	if(is_file($uploadDir.$originname)){
		$attachmentDir = $_SC['attachdir'];
		
		$newfilename='./topicattachment/';
		mkdir($_SC['attachdir'].$newfilename);
		
		$newfilename = $newfilename.$_SGLOBAL['timestamp'].'.'.$suffix;
		copy($uploadDir.$originname, $attachmentDir.$newfilename); //拷贝到新目录
		unlink($uploadDir.$originname); //删除旧目录下的文件
		return $newfilename;
	}
...
function mtag_attachment_save($originname,$tagid) {
	global $_SGLOBAL, $_SCONFIG, $space, $_SC;
	...
	if(is_file($uploadDir.$originname)){
		...
		$newfilename='./mtag/'.$tagid.'/';
		mkdir($_SC['attachdir'].$newfilename);
		
		$newfilename = $newfilename.$_SGLOBAL['timestamp'].'-'.$originname;
		copy($uploadDir.$originname, $attachmentDir.$newfilename); //拷贝到新目录
		unlink($uploadDir.$originname); //删除旧目录下的文件
		return $newfilename;
	}
...
function hot_update($idtype, $id, $hotuser) {
	...
	switch ($idtype) {
		...
		//	$_SGLOBAL['db']->query("UPDATE ".tname('mtag_thread')." SET hot=hot+1 WHERE tid='$id'");
			break;
		case 'picid':
...
function internal_mail($grouptype, $subject, $message=''){
	$addressarray = array();
	
	if($grouptype == 'MAIL_TOPIC_AUDITOR'){
		$addressarray[] = '846528446@echodrama.com';//爱爱
		$addressarray[] = '156838991@echodrama.com';//superyang_xp
		$addressarray[] = '631909395@echodrama.com';//九月君
		$addressarray[] = '842143795@echodrama.com';//卡特
		$addressarray[] = '928847685@echodrama.com';//J
		$addressarray[] = '525725769@echodrama.com';//xinting
	}
	
	foreach($addressarray as $address){
		smail(0, $address, $subject, $message, '');
	}
}
