...
if($view == 'userapp') {
	...
	$noticetypes = array(
		...
		'wall' => "&raquo;留言",
		'topic' => "&raquo;广播剧通知",
	    'topic_audit' => "&raquo;广播剧上传审核通知",
	    'topic_admin_audit' => "&raquo;广播剧审核处理通知",
	    'topic_delete' => "&raquo;广播剧删除通知",
		'topiccomment' => "&raquo;广播剧新评论通知",
	    'mtag_manage' => "&raquo;群组内部通知",
	    'mtag_thread_new' => "&raquo;群组新话题通知",
	...
	if(empty($type)){
		...
		$pro_actives = array($_GET['type'] => ' style="font-weight:bold;"');
	...
	if($count) {
		...
		//分页
		$multi = multi($count, $perpage, $page, "space.php?do=notice&type=".$_GET['type']);
	}
	...
	if($newids) {
		...
		updatetable('session', array('reload'=>1), array('uid'=>$_SGLOBAL['supe_uid']));
	}
