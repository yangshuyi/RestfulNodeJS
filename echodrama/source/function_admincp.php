...
function getwheres($intkeys, $strkeys, $randkeys, $likekeys, $pre='') {
	...
	foreach ($likekeys as $var) {
		...
		//TODO update the param from 1 to 0 remove BINARY for case-insensitive
		if(strlen($value)>0) {
...
//获取限制条件
function getpostwheres($intkeys, $strkeys, $randkeys, $likekeys, $pre='') {
	...
	foreach ($randkeys as $vars) {
		$value1 = isset($_POST[$vars[1].'1'])?$vars[0]($_POST[$vars[1].'1']):'';
		$value2 = isset($_POST[$vars[1].'2'])?$vars[0]($_POST[$vars[1].'2']):'';
		if($value1) {
			$wherearr[] = "{$pre}{$vars[1]}>='$value1'";
			$urls[] = "{$vars[1]}1=".rawurlencode($_POST[$vars[1].'1']);
		}
		if($value2) {
			$wherearr[] = "{$pre}{$vars[1]}<='$value2'";
			$urls[] = "{$vars[1]}2=".rawurlencode($_POST[$vars[1].'2']);
		}
	}
	
	foreach ($likekeys as $var) {
		$value = isset($_POST[$var])?stripsearchkey($_POST[$var]):'';
		if(strlen($value)>1) {
...
function getstatistics() {
	...
	$statistics = array(
		...
		'threadnum' => getcount('mtag_thread', array()),
