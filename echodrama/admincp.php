...
if($_SGLOBAL['administrator'] == 1){
	$acs = array(
		...
		array('tag', 'event', 'space'),
		array('cache', 'network', 'profilefield', 'jobclass', 'click', 'task', 'censor', 'stat', 'block', 'cron', 'app', 'log'),
		array('topic', 'topicrecommand', 'topicalbum','cover', 'video', 'radio', 'tone', 'cvweibo'),
		array('mtag'),
		array('sitepm', 'announcement', 'task', 'chart'),
		array('cover','video','feed', 'blog', 'album', 'pic', 'comment', 'doing', 'share', 'poll')
	);
}
else{
	$acs = array(
		...
		array(),
		array(),
		array('feed', 'blog', 'album', 'pic', 'comment', 'doing', 'share', 'poll')
	);
}
...
if(empty($_GET['ac']) || (!in_array($_GET['ac'], $acs[0]) && !in_array($_GET['ac'], $acs[1]) && !in_array($_GET['ac'], $acs[2]) && !in_array($_GET['ac'], $acs[3]) && !in_array($_GET['ac'], $acs[4]) && !in_array($_GET['ac'], $acs[5]) && !in_array($_GET['ac'], $acs[6]))) {
	$ac = 'index';
} else {
	$ac = $_GET['ac'];
}
