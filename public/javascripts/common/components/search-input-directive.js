...
angular.module('common.components')
    .directive('searchField', ['$timeout', function ($timeout) {
        return {
            ...
            template: '{{configFilterInput}}<div class="input-group search-input">\
                                        <input no-dirty-check type="text" class="form-control ccc-field" ng-model="inputValue" placeholder="{{plcHolder}}" class="search-input" ng-keydown="bindKeyPressEvent()" ng-keyup="bindKeyUpEvent(inputValue)">\
                                        <span ng-show="inputValue" class="glyphicon glyphicon-remove filter-clean-btn" ng-click="bindReturnSearch()"></span>\
                                        <span class="input-group-btn">\
                                            <button class="btn btn-default search-field-btn" type="button" ng-click="bindClickEvent()">\
